...
- **Task 6:** fullSync chains a flush on success when a write landed mid-sync (same stranding class as the Task 5 rerun-flag fix; chained only when lastError is null so a dead network cannot hot-loop); FakeBackend gained a pullGate Completer; one extra test covers the chained flush (engine 18 tests, suite 41).
- **Task 12:** provisioning deferred by decision on 2026-07-10 — the org's two free Supabase slots are used, so a dedicated project is $10/mo, not worth paying months before the Oct 1 launch. Code side is complete and verified: `flutter build web` compiles, full suite green (47 tests), analyze at baseline, offline/local mode runs with no backend. Live-backend recipe + e2e checklist handed off in `docs/PROVISIONING.md`. Local scaffolding (env.example.json, gitignored env.json, README `--dart-define-from-file`) committed.
